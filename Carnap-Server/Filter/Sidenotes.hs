...
pureMakeSideNotes i@(Note (Para ((Str (s:ss):is)):bs)) n = 
        case s of
...
processNote (Note (Para ((Str (s:ss):is)):bs)) = Span sidenoteAttr (breakInlines $ Para ((Str ss):is):bs)
processNote i = i

rawHead n = RawInline "html" $ "<label for=\"sn" ++ show n ++ "\" class=\"margin-toggle sidenote-number\"></label>"
                                ++ "<input type=\"checkbox\" id=\"sn" ++ show n ++ "\" class=\"margin-toggle\"/>"
